...
Because some of the studies in TOPMed have related individuals, we need to fit a null model to account for the correlation in the data.
In this case, because the phenotype is quantitative, we will use a linear model.

We use the `GENESIS` R package for fitting the null model.
...
The first step in fitting the null model is to create an AnnotatedDataFrame.
...
The `GENESIS` code to fit the null model also requires a `sample.id` column.
...
We will first fit a null model that allows us to see if the mean of the height phenotype is different by study after adjusting for other covariates.
...
The null model also shows the differences in mean height by study.
...
```{r}
annot$residuals <- mod_2$resid.marginal
```
